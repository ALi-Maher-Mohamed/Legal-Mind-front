...
import { Heart, Edit, Trash2, ArrowRight, Image as ImageIcon } from 'lucide-react';
...
export default function BlogDetailsPage() {
  ...
  const handleDelete = async () => {
    const confirmDelete = window.confirm('هل أنت متأكد من حذف هذا المقال نهائياً؟');
    ...
    return (
      <div className="w-full max-w-4xl mx-auto p-6 text-center" dir="rtl">
        <DashPanel className="py-20">
          ...
          <button onClick={() => router.push('/dashboard/gazette')} className="text-blue-600 underline">
            العودة للجريدة
          </button>
        </DashPanel>
      </div>
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-4xl mx-auto pb-10" dir="rtl">
      <button 
          ...
          {blog.tags && blog.tags.length > 0 && (
            <div className="mt-10 flex flex-wrap gap-2">
              {blog.tags.map((tag, index) => (
                <span key={index} className="px-3 py-1 bg-slate-100 text-slate-600 text-sm rounded-lg">
                  #{tag}
                </span>
              ))}
            </div>
