...
export default function PublicBlogDetailsPage() {
  ...
  const handleSpeech = () => {
    ...
    const utter = new SpeechSynthesisUtterance(`${blog.title}. ${stripHtml(blog.content)}`);
    utter.lang = 'ar-EG';
    utter.rate = 0.95;
  ...
  const cover = blog ? getCoverImage(blog) : null;
  const avatar = blog ? getAuthorAvatar(blog) : null;
  const authorName = blog ? getAuthorName(blog) : '';
    ...
    <>
      ...
      <main className="min-h-screen bg-background px-4 py-6 pt-24 sm:px-6 sm:py-8 lg:px-8" dir="rtl">
        {isLoading ? (
          ...
          <div className="mx-auto max-w-3xl rounded-xl border bg-white py-16 text-center dark:bg-card">
            <p className="mb-4 font-bold text-danger">{error || 'لم يتم العثور على المقال.'}</p>
            <Link href={ROUTES.gazette} className="text-sm font-bold text-brand underline">
              {c.backToIndex}
            </Link>
          </div>
        ) : (
          <div className="mx-auto grid w-full max-w-6xl grid-cols-1 gap-6 lg:grid-cols-12">
            <article className="rounded-xl border border-[#c4c6cf] bg-white p-5 shadow-[0_4px_20px_rgba(26,54,93,0.05)] dark:border-white/10 dark:bg-card sm:p-8 lg:col-span-8">
              ...
              <span className="mb-3 inline-flex items-center gap-1.5 rounded-md border border-[#002045]/15 bg-[#002045] px-2.5 py-1 text-[11px] font-bold text-white">
                ...
                {canManage ? (
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      onClick={() =>
                        gate.requireAuth(() => {
                          router.push(`/dashboard/gazette/create?edit=${blogId}`);
                        })
                      }
                      className="inline-flex items-center gap-2 rounded-lg border border-[#c4c6cf] px-4 py-2 text-xs font-bold text-[#002045] dark:border-white/15 dark:text-foreground cursor-pointer"
                    >
              ...
              <BlogComments blogId={blogId} onRequireLogin={() => gate.requireAuth()} />
            </article>
            ...
            <aside className="space-y-4 lg:col-span-4">
              <div className="rounded-xl bg-[#f1f4f6] p-5 dark:bg-white/5">
                ...
                <div className="space-y-3">
                  {related.map((item) => (
                    <Link
                      key={getBlogId(item)}
                      href={ROUTES.gazetteArticle(getBlogId(item))}
              ...
              <button
                ...
                className="flex w-full items-center justify-center rounded-xl bg-[#002045] px-4 py-3.5 text-sm font-bold text-white transition hover:opacity-90 cursor-pointer"
              >
                {c.consultCta}
              </button>
            </aside>
