...
async function imageToPngDataUrl(img: HTMLImageElement, size = 512): Promise<string> {
...
function buildDocumentHtml(bodyHtml: string) {
  return `
  <div id="lm-pdf-root" dir="rtl" lang="ar" style="
    ...
    overflow: hidden;
  ">
    <div style="position: relative; z-index: 1; padding: 36px 48px 28px;">
...
function injectPdfStyles(root: HTMLElement) {
  ...
  style.textContent = `
    #lm-pdf-root .lm-pdf-body > :first-child {
      margin-top: 0;
    }
...
function addPageChrome(
  ...
  // Soft watermark — centered logo + brand wordmark, very light
  const wmSize = 82;
  const wmX = (pageWidth - wmSize) / 2;
  const wmY = (pageHeight - wmSize) / 2 - 14;

  pdf.saveGraphicsState();
  pdf.setGState(pdf.GState({ opacity: 0.055 }));
  pdf.addImage(watermarkDataUrl, 'PNG', wmX, wmY, wmSize, wmSize, undefined, 'FAST');
  pdf.setTextColor(0, 62, 199);
  pdf.setFont('helvetica', 'bold');
  pdf.setFontSize(22);
  pdf.text('LEGAL MIND', pageWidth / 2, pageHeight / 2 + 42, {
    align: 'center',
    angle: 32,
  });
  ...
  pdf.setFontSize(8);
  pdf.text(`${pageNumber} / ${totalPages}`, pageWidth / 2, pageHeight - 7, { align: 'center' });
}

export async function exportContractPdf(options: ExportContractPdfOptions): Promise<void> {
  ...
  host.innerHTML = buildDocumentHtml(bodyHtml);
  ...
  try {
    ...
    for (let page = 0; page < totalPages; page += 1) {
      ...
      pdf.addImage(pageData, 'JPEG', marginX, marginTop, imgWidth, drawHeight, undefined, 'FAST');
