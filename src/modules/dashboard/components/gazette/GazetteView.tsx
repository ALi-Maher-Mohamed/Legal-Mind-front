...
export default function GazetteView() {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        setIsLoading(true);
        const response = await fetch('http://localhost:5001/api/blogs?page=1&limit=10&sort=newest');
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-7xl mx-auto pb-10" dir="rtl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">الجريدة القانونية</h1>
          <p className="text-sm text-slate-500 mt-1">تصفح أحدث المقالات والتحليلات القانونية.</p>
        </div>
        <Link 
      ...
      {isLoading ? (
        ...
        <DashPanel className="flex flex-col items-center justify-center py-20 text-slate-500">
          ...
          <h2 className="text-xl font-semibold text-slate-700 mb-2">لا توجد مقالات حتى الآن</h2>
          <p className="text-sm">كن أول من يضيف محتوى للجريدة!</p>
        </DashPanel>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
          {blogs.map((blog) => (
            <DashPanel key={blog._id} padded={false} className="overflow-hidden flex flex-col transition-transform hover:scale-[1.02] duration-300">
